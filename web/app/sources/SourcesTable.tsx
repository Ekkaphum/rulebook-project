...
export default function SourcesTable({ rows }: { rows: SourceRow[] }) {
  ...
  return (
    <>
      <div style={{ display: "grid", gap: ".9rem", marginBottom: "1.5rem" }}>
        <SearchBox value={q} onChange={setQ} placeholder="Search titles and URLs…" />
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: ".85rem", minWidth: 680 }}>
          <thead>
            <tr>
              {["ID", "Type", "Title", "Years", "Link"].map((h) => (
      ...
      {filtered.length === 0 && <p style={{ color: "var(--ink-3)", marginTop: "1rem" }}>Nothing matches that search.</p>}
    </>
  );
}
